...
export class FiscalizacionStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props?: FiscalizacionStackProps) {
    ...
    const generateExportName = (resource: string): string => {
      return `${project.replace(/_/g, '-')}-${client}-${useCase}-${resource}-${stage}-fisc`;
    };
    ...
    const testOracleNodejsLambda = new lambda.DockerImageFunction(this, 'TestOracleNodejsLambda-f', {
      functionName: generateResourceName('lambda_test_oracle_nodejs-f'),
      code: lambda.DockerImageCode.fromImageAsset(path.join(__dirname, 'lambdas/test-oracle-nodejs-container-fisc'), {
